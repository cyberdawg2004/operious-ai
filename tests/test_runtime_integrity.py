...
def test_reranker_registry_iterates_in_sorted_name_order() -> None:
    """Same invariant for the RAG reranker registry."""
    ...
    class _Stub(BaseReranker):
        ...
        async def rerank(self, *a: Any, **kw: Any) -> Any:  # pragma: no cover
            raise NotImplementedError

    reg = RerankerRegistry()
    reg.register(_Stub("zulu"))
    reg.register(_Stub("alpha"))
    reg.register(_Stub("mike"))

    names = tuple(r.name for r in reg)
    assert names == ("alpha", "mike", "zulu")


def test_every_registry_publishes_sorted_names() -> None:
    ...
    assert WorkflowRegistry().names() == ()
    assert ProviderRegistry().names() == ()
    assert EmbeddingProviderRegistry().names() == ()
    assert VectorProviderRegistry().names() == ()
    assert RerankerRegistry().names() == ()
