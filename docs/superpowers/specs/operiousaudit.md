...
Date: 2026-05-31
Current-state update: 2026-07-05, after post-Phase-2.2 security hardening, Stage 3b connector proof tooling, default-auth/stub/CORS/seeded-replay/action-approval fixes, MVP-1 extraction-schema agnosticism, MVP-2 generic action-registry agnosticism, MVP-3 through MVP-9 governed intelligence work, tenant-config payload validation, frontend schema/role editors, Phase 0 release-hygiene / no-stub production-fail-closed review, partial S-10 live-production evidence capture, and the follow-up `2b7cacd` S-10 evidence/test cleanup review.
Auditor: Codex static architecture/security review
Audited HEAD: `2b7cacd` (`fix(pyright/s10): fix migration dialects imports, s10 full evidence capture`)
Working-tree note: this update reviews committed changes through `2b7cacd`, including the committed MVP/intelligence-layer work, CORS fail-closed fallback removal, malformed auth/authority parse coarsening, callback HMAC protection, raw-body callback wiring, S-10 v235 evidence files, zero-warning pyright cleanup, compliance/ops docs, the RLS nullable-tenant hardening invariant, migration dialect import cleanup, direct-ASGI authority coarsening test harness, system-smoke env isolation attempt, and the 2026-07-05 expanded S-10 capture artifact. The working tree was clean before this audit/manifest update. Local ignored artifacts `audit-artifacts/`, `aws/`, `awscliv2.zip`, and `infra/` are still not production readiness evidence. Generated `__pycache__`/`.pyc` files must remain uncommitted. `docs/operiousaudit.md` was moved to `docs/superpowers/specs/operiousaudit.md`.
...
- S-10 is still open, but it is no longer "no live proof." A live-production bundle is archived for Fly release v235 / commit `c65fda5`: direct spoof rejection, production readiness READY, one-table production-role RLS isolation, workers/DLQ proof, Auth0 `/auth/me` verified-claims proof in the expanded capture, and worker/smoke pipeline progress. It does not close S-10 for current audited HEAD `2b7cacd` because the exact deployed release evidence is still tied to `c65fda5`, the expanded capture marks CORS `PARTIAL`, rate limiting only `OBSERVED` on `/health`, full production FORCE-RLS catalog proof is still missing, and the current smoke still fails final diagnostic completion on Anthropic HTTP 400.

The blocking issue has shifted again. It is no longer "there are no controls," "there are no side-effect ledgers," "manager approvals can silently use fake-success stubs," "connector credential/RLS work is only local," "generic connectors are only a model," "the product can only express warranty/refund e-commerce fields," "CORS can silently fall back to hardcoded origins," or "#25 malformed auth parse leaks are still open." The blocker is current-head live proof, operational completeness, provider maturity, and repo/infra hygiene. `S-10` remains open because the archived live bundle was captured against deployed `c65fda5` / Fly v235, while the audited repo HEAD is `2b7cacd`, and because the expanded bundle still has partial CORS evidence, non-proving rate-limit evidence, no full production FORCE-RLS catalog proof, and no clean provider-completing smoke. Remaining review caveats: data-protection controls need KMS/retention/legal-operation proof before compliance claims, live S3/Anthropic/provider drills are not clean, the 2026-07-05 smoke trace reached the diagnostic worker but failed all retries on an Anthropic HTTP 400, nullable-tenant RLS policy semantics need continued DB-level hardening, the new frontend advanced raw-JSON override is display-only unless submit handling is wired, dead duplicate Python trees and local ignored AWS/Terraform artifacts remain, and the current smoke test suite still has a local DB/resource-lifespan timeout that is not counted as a pass.
...
Repository traversal now finds 2,794 discoverable files via `rg --files`. This reflects the tracked hardening/MVP/S-10/compliance commits plus the committed 2026-07-05 expanded S-10 capture artifact, while local ignored `aws/`, `awscliv2.zip`, `infra/`, and `audit-artifacts/` still exist on disk.
...
| `2113e01` / `2d42cd0` / `9b29e0a` / `c65fda5` / `c1f39f5` / `244e32e` / `57e5b88` / `cccd538` / `2b7cacd` | Current committed Phase-1/2 extension: MVPs 1-9 committed, malformed auth/authority parse coarsening code-closed, fulfillment callback HMAC added, raw callback body/repository wiring fixed, hardcoded CORS fallback removed, S-10 v235 evidence captured, pyright warning count reduced to zero, ops/compliance docs added, RLS nullable-tenant static invariant linted, migration dialect imports cleaned for pyright, direct-ASGI coarsening tests committed, system-smoke env isolation attempted, and expanded S-10 capture archived. |
| Current audit/manifest update | This audit refresh and the manifest label correction are doc/evidence edits after reviewing `2b7cacd`; no production code is changed by this update. |
...
| Backend type check | Current pyright at audited HEAD `2b7cacd`: `pyright -p pyrightconfig.json` -> `0 errors, 0 warnings, 0 informations` |
...
| Current post-`2b7cacd` focused verification | Codex rerun: ruff on touched migration/test files -> `All checks passed!`; `test_authority_context_coarsening.py`, `test_production_readiness.py`, `test_check_production_readiness_script.py` -> `33 passed in 1.43s`; pyright -> `0 errors, 0 warnings, 0 informations`; S-10 JSON files validate |
...
| System smoke tests | Current `2b7cacd` rerun of `test_system_smoke.py` timed out at 120s after one passing dot; verbose 40s rerun shows `test_health_endpoint_live` passed before the process timed out entering the Postgres-marked smoke path | Not counted as a pass. The new env-isolation fixture reduces cross-module contamination risk, but this module is still not deterministic enough to use as release evidence until the DB/resource-lifespan path is fixed or explicitly skipped when unavailable. |
...
Residual gaps update: S-01 through S-09 are no longer open original vulnerabilities. They are closed or closed-with-residuals as described below. The previous tenant-config `applied_by` regression is fixed and proven by DB-backed tests. The previous action-approval stub DI residual is fixed in current code and has a regression guard. The connector/OMS/RLS work is committed and DB-proven, Stage 3 generic connector calls now have unit plus live external-call proof, and the Phase 0 tenant-config DB/router proof is clean. The newer Phase-1/2 hardening commits are materially correct in the reviewed source. The hardcoded CORS fallback and #25 malformed-parse leak are now code-closed. S-10 remains open because the live evidence is not current-head-complete, CORS is only partially proven, rate limiting is not live-proven on a protected route, full production FORCE-RLS catalog proof is missing, and the smoke trace does not complete because Anthropic returns HTTP 400. Remaining forensic/current residuals are nullable-tenant/RLS DB hardening, dead duplicate Python trees, owner-session/RLS-off assumptions, duplicate frontend clients, local ignored infra/AWS artifacts, and deterministic smoke/release evidence gaps.
...
| Enterprise readiness | 43 | 86 | Not ready | Security/governance primitives, domain agnosticism, production-fail-closed defaults, compliance docs, zero-warning type state, and partial S-10 evidence improved readiness, but current-head S-10, clean provider smoke, formal compliance operations, DR/SLO evidence, full live CORS/RLS/rate-limit proof, duplicate-tree/repo hygiene, and production provider integrations cap readiness. The `2b7cacd` evidence cleanup does not raise this score because it records CORS/rate-limit/provider-smoke gaps. |
...
- The same expanded capture is not S-10 closure evidence: CORS preflight is marked `PARTIAL`, rate limiting is only `OBSERVED` after 30 unauthenticated `/health` requests, the smoke still fails final diagnostic completion on Anthropic HTTP 400, and the manifest's exact deployed commit remains `c65fda5` rather than audited HEAD `2b7cacd`.
...
Status: open. The boot gate is committed and partial live production evidence exists, but S-10 is not closed for current HEAD `2b7cacd`.
...
- Current-head live production Auth0 `/auth/me` token claim mapping and role assignment. A 2026-07-05 proof exists inside the expanded Fly v235 / `c65fda5` bundle, not for audited HEAD `2b7cacd`.
- Current-head production trusted ingress/direct spoof rejection. A 2026-07-05 proof exists for Fly v235 / `c65fda5`, not for audited HEAD `2b7cacd`.
- Full production FORCE-RLS catalog proof. A 2026-07-05 one-table RLS proof exists, but it is not full catalog evidence.
- Current-head production worker/DLQ/health checks. A 2026-07-05 worker/DLQ proof exists for Fly v235 / `c65fda5`, not for audited HEAD `2b7cacd`.
...
Score movement after `2b7cacd`: unchanged. Security remains `98`; Enterprise readiness remains `86` because S-10 is still open and the new evidence explicitly leaves CORS, rate-limit, current-head release proof, FORCE-RLS catalog proof, and clean provider smoke unresolved.
