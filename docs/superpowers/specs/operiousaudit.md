...
Date: 2026-05-31
Current-state update: 2026-07-08, after post-Phase-2.2 MCP connector governance, connector credential KMS/RLS work, manager assistant, inbox, cross-channel identity/merge work, command-center UX changes, CI stabilization commits through `74ca1f1`, and the current Codex review/verification pass.
Auditor: Codex static architecture/security review
Audited HEAD: `74ca1f1` (`fix(ci): real fixes for remaining 5 CI failures (Group 6)`)
Working-tree note: this update reviews committed changes through `74ca1f1`, including the Phase-2.2 MCP/manager/inbox/connector-credential work and CI follow-up commits. The only pre-existing local code modification observed during this review was `apps/command-center2/frontend/components/dashboard-shell.tsx`, adding dashboard metadata entries for `inbox` and `assistant`; this audit update does not revert or modify that working-tree change. Local ignored artifacts `audit-artifacts/`, `aws/`, `awscliv2.zip`, and `infra/` are still not production readiness evidence. Generated `__pycache__`/`.pyc` files must remain uncommitted. `docs/operiousaudit.md` was moved to `docs/superpowers/specs/operiousaudit.md`.
...
- Current-head Phase-2.2 adds real MCP connector machinery: per-connector encrypted credential storage, `connector_credentials` RLS, OAuth state HMAC hardening, SSRF-pinned MCP preview/list/invoke paths, tenant-scoped manager assistant and inbox APIs, and MCP execution-policy tests. These are substantial controls, not merely UI scaffolding.
- Current-head review also found two release-blocking quality issues: the MCP server registration route passes `McpToolEntry` model instances into a service validator that requires dictionaries, so a valid API request can be rejected before persistence; and `pyright -p pyrightconfig.json` now fails with 22 errors / 46 warnings. These do not reopen S-01 through S-09, but they prevent calling the current head production-grade.
- MCP custom-tool governance intentionally allows tenant-declared `execution_policy=auto_execute` for money/goods/service-commitment tools, logging an audit warning and delegating downstream authorization to the tenant system. That is coherent as a tenant-autonomy product decision, but it is not the strict platform-enforced human-approval floor previously used for generic REST money/goods actions. If the target is "no tolerance" irreversible-action safety, this should be changed to require Operious approval for money/goods/service commitments regardless of MCP `auto_execute`.
...
- Type hygiene had improved materially at the previous audited head, but current `74ca1f1` reopens the quality gate: `pyright -p pyrightconfig.json` reports `22 errors, 46 warnings`.
...
- S-10 is still open, but it is no longer "no live proof." A live-production bundle is archived for Fly release v235 / commit `c65fda5`: direct spoof rejection, production readiness READY, one-table production-role RLS isolation, workers/DLQ proof, Auth0 `/auth/me` verified-claims proof in the expanded capture, and worker/smoke pipeline progress. It does not close S-10 for current audited HEAD `74ca1f1` because the exact deployed release evidence is still tied to an older commit, the expanded capture marks CORS `PARTIAL`, rate limiting only `OBSERVED` on `/health`, full production FORCE-RLS catalog proof is still missing, and the current smoke still fails final diagnostic completion on Anthropic HTTP 400.

The blocking issue has shifted again. It is no longer "there are no controls," "there are no side-effect ledgers," "manager approvals can silently use fake-success stubs," "connector credential/RLS work is only local," "generic connectors are only a model," "the product can only express warranty/refund e-commerce fields," "CORS can silently fall back to hardcoded origins," or "#25 malformed auth parse leaks are still open." The current blockers are current-head live proof, the MCP registration bug, the failed type check, operational/provider maturity, and repo/infra hygiene. `S-10` remains open because the archived live bundle was captured against deployed `c65fda5` / Fly v235, while the audited repo HEAD is `74ca1f1`, and because the expanded bundle still has partial CORS evidence, non-proving rate-limit evidence, no full production FORCE-RLS catalog proof, and no clean provider-completing smoke. Remaining review caveats: data-protection controls need KMS/retention/legal-operation proof before compliance claims, live S3/Anthropic/provider drills are not clean, the 2026-07-05 smoke trace reached the diagnostic worker but failed all retries on an Anthropic HTTP 400, nullable-tenant RLS policy semantics need continued DB-level hardening, MCP `auto_execute` for money/goods needs a product/security decision, dead duplicate Python trees and local ignored AWS/Terraform artifacts remain, and the current smoke test suite still has local timeout behavior that is not counted as a pass.
...
Repository traversal now finds 2,827 discoverable files via `rg --files`. This reflects the tracked hardening/MVP/S-10/compliance commits, the committed 2026-07-05 expanded S-10 capture artifact, and the current Phase-2.2 MCP/manager/inbox/connector-credential work. Local ignored `aws/`, `awscliv2.zip`, `infra/`, and `audit-artifacts/` still exist on disk.

Top-level inventory:

| Area | Count | Notes |
| --- | ---: | --- |
| `apps` | 2,608 | Backend, command center, platform console, marketing, connector, attachment, approval, warranty/refund, MCP connector, manager-assistant/inbox, intelligence/governed-agent, and test changes |
...
| `2113e01` / `2d42cd0` / `9b29e0a` / `c65fda5` / `c1f39f5` / `244e32e` / `57e5b88` / `cccd538` / `2b7cacd` | Prior committed Phase-1/2 extension: MVPs 1-9 committed, malformed auth/authority parse coarsening code-closed, fulfillment callback HMAC added, raw callback body/repository wiring fixed, hardcoded CORS fallback removed, S-10 v235 evidence captured, pyright warning count reduced to zero at that prior point, ops/compliance docs added, RLS nullable-tenant static invariant linted, migration dialect imports cleaned for pyright, direct-ASGI coarsening tests committed, system-smoke env isolation attempted, and expanded S-10 capture archived. |
| Prior audit/manifest update | The previous audit refresh and manifest label correction were doc/evidence edits after reviewing `2b7cacd`; no production code was changed by that update. |
| `4762b66` / `5946557` / `3c99f78` / `dce775b` / `632632e` | Smoke TCP reachability guard, Command Center UX overhaul, and MVP-7 cross-channel identity detection/confirmed merge flow. |
| `5b4ef0b` / `ee3c415` / `a56c3c0` / `c932874` / `9a96e79` / `6c47073` / `a043ddd` | Phase-2.2 security and product tranche: MCP connector governance, manager assistant, inbox, connector credential KMS/RLS, SSRF-rebinding closure on the agent MCP path, OAuth redirect/base-url hardening, prompt-injection isolation, operator gates, platform auto-approve ceiling, and capability-gated manager surfaces. |
| `294c9eb` / `488c9a6` / `19f3268` / `ed90dcc` / `40f8dd1` / `8e8a0c4` / `0fa0ecb` / `5683785` / `000f634` / `b8e2aaa` / `abd905a` / `74ca1f1` | CI stabilization after the Phase-2.2 tranche: MCP test skip behavior, fixture/signature fixes, approval registry wiring, repair-dispatch connector detection, moving stub tool classes out of production app modules into test support, extraction parsing and connector error handling fixes, knowledge-ingestion test isolation, and remaining CI failure fixes. Current review still finds a route-level MCP registration bug and a non-clean pyright state at `74ca1f1`. |
...
| Prior backend type check | Pyright at prior audited HEAD `2b7cacd`: `pyright -p pyrightconfig.json` -> `0 errors, 0 warnings, 0 informations`; this is superseded by the current-head failing pyright row below |
| S-10 partial live production evidence bundle | Committed evidence captured against Fly v235 / commit `c65fda5`: `2026-07-05-spoofing-pass.json`, `2026-07-05-readiness-ready.json`, `2026-07-05-rls-pass.json`, `2026-07-05-workers-dlq-pass.json`, `2026-07-05-smoke-trace.json`, and `2026-07-05-s10-full-capture.json`; JSON validates |
| S-10 expanded capture review | `2026-07-05-s10-full-capture.json` records spoof rejection PASS, Auth0 `/auth/me` PASS, queue/worker PASS, and production boot PASS, but CORS is `PARTIAL`, rate limiting is only `OBSERVED` on `/health`, and the smoke trace still fails final diagnostic completion on Anthropic HTTP 400 |
...
| Prior post-`2b7cacd` focused verification | Codex rerun at the prior head: ruff on touched migration/test files -> `All checks passed!`; `test_authority_context_coarsening.py`, `test_production_readiness.py`, `test_check_production_readiness_script.py` -> `33 passed in 1.43s`; pyright -> `0 errors, 0 warnings, 0 informations`; S-10 JSON files validated. Superseded by current-head verification rows below |
...
| Current-head MCP connector governance and real-server path | Codex rerun at `74ca1f1`: non-e2e MCP/governance/credential/custom-money subset produced `56 passed, 3 skipped`; `test_mcp_e2e_real_server.py` initially failed only because the sandbox denied local socket creation, then passed outside the sandbox with approval: `7 passed in 3.50s` |
| Current-head security/manager/MVP non-DB bundle | Codex rerun at `74ca1f1`: `test_security_fixes_tier1.py`, `test_security_fixes_tier2.py`, `test_manager_assistant_cross_tenant_isolation.py`, `test_mvp7_identity_detection.py`, `test_mvp7_merge_flow.py`, `test_non_commerce_vertical.py` -> `115 passed in 4.13s` |
| Current-head connector credential lifecycle | Codex rerun at `74ca1f1`: `test_connector_credential_lifecycle.py` -> `13 passed in 1.62s` |
| Current-head migration state | Before rerun, local `operious_test` reported `alembic current` -> `0097_mvp7_customer_identity_id`, which caused DB-backed connector-config tests to fail on missing `connector_configs.callback_hmac_secret`; Codex upgraded only the local test DB through `0104_connector_credentials_rls` |
| Backend lint for current Python code | Codex rerun at `74ca1f1`: `ruff check apps/backend/app apps/backend/tests` -> `All checks passed!` |
| Backend type check after current cleanup | Not clean at `74ca1f1`: `pyright -p pyrightconfig.json` -> `22 errors, 46 warnings`; key errors are in MCP client/router typing, undefined `InboxService`/`ManagerAssistantService` type imports in dependency wiring, `Settings.command_center_url` type access, manager-assistant date typing, and an unused MCP helper |
...
| System smoke tests | Prior `2b7cacd` rerun of `test_system_smoke.py` timed out at 120s after one passing dot; current-head rerun also timed out after `test_health_endpoint_live` passed | Not counted as a pass. The env-isolation/TCP-guard work reduces cross-module contamination risk, but this module is still not deterministic enough to use as release evidence until the DB/resource-lifespan path is fixed or explicitly skipped when unavailable. |
...
| Current-head MCP server registration route | Static/source check plus direct serialization probe show `McpServerRegisterRequest.mcp_tools` contains `McpToolEntry` objects, while `TenantConfigChangeRequestService._validate_mcp_server_payload()` requires each item to be a `dict`; `canonical_sha256()` also cannot JSON-serialize `McpToolEntry` | Real current bug. Convert `request.mcp_tools` to `[tool.model_dump()]` before service call and add a FastAPI route regression test for `POST /{tenant_id}/mcp/servers`. |
| Current-head backend type check | `pyright -p pyrightconfig.json` -> `22 errors, 46 warnings` | Real current quality gate failure. Not production-grade until fixed or intentionally baselined. |
| Current-head DB-backed combined verification | Initial run failed because local `operious_test` was still at `0097` and lacked `connector_configs.callback_hmac_secret`; after upgrading to `0104`, the combined DB suite emitted only pass markers but still hit the 300s timeout before summary | Do not count as full DB-suite proof. Keep individual passing evidence, require CI/normal-shell full DB run at migration head. |
| Current-head smoke/action bundle | `test_system_smoke.py` passed `test_health_endpoint_live` then timed out under the 120s local command; the action/SSRF/authority bundle also timed out after partial progress | Not counted as release evidence. Split/fix the slow path or make DB/socket prerequisites explicit. |
...
| Operator HTTP probe helper | `apps/backend/scripts/test_http_probe.sh` is tracked as an operator-configurable probe helper | Not S-10 production evidence by itself. It is excluded from Python lint/type proof because it is shell and still needs explicit operator-run output before being cited as live proof. |

Residual gaps update: S-01 through S-09 are no longer open original vulnerabilities. They are closed or closed-with-residuals as described below. The previous tenant-config `applied_by` regression is fixed and proven by DB-backed tests. The previous action-approval stub DI residual is fixed in current code and has a regression guard. The connector/OMS/RLS work is committed and DB-proven, Stage 3 generic connector calls now have unit plus live external-call proof, and the Phase 0 tenant-config DB/router proof was clean at that point. The newer Phase-2.2 hardening commits are materially stronger overall, but current-head review found the MCP registration route bug, failed pyright gate, and MCP money/goods `auto_execute` policy question listed below. The hardcoded CORS fallback and #25 malformed-parse leak are now code-closed. S-10 remains open because the live evidence is not current-head-complete, CORS is only partially proven, rate limiting is not live-proven on a protected route, full production FORCE-RLS catalog proof is missing, and the smoke trace does not complete because Anthropic returns HTTP 400. Remaining forensic/current residuals are nullable-tenant/RLS DB hardening, dead duplicate Python trees, owner-session/RLS-off assumptions, duplicate frontend clients, local ignored infra/AWS artifacts, and deterministic smoke/release evidence gaps.

## Current-Head Review Findings - 2026-07-08

| Finding | Status | Evidence | Required change |
| --- | --- | --- | --- |
| F-2026-07-08-01 MCP server registration route rejects valid API payloads | Open | `apps/backend/app/api/v1/routers/tenant.py:1511-1516` passes `request.mcp_tools` directly; `apps/backend/app/services/tenant_config_change_request_service.py:1942-1946` rejects non-dict tool entries; direct probe confirmed `McpToolEntry` is not a `dict` and is not JSON serializable for canonical hashing | Convert nested Pydantic entries to dictionaries before calling the service; add route-level test for `POST /{tenant_id}/mcp/servers` with a normal JSON request body |
| F-2026-07-08-02 Backend type gate is no longer clean | Open | `pyright -p pyrightconfig.json` at `74ca1f1` returned `22 errors, 46 warnings` | Fix type errors or create an explicit reviewed baseline. Until then, remove any audit claim that current head has zero-warning pyright |
| F-2026-07-08-03 S-10 is still not current-head-closed | Open | Latest archived S-10 bundle is tied to older Fly v235 / `c65fda5`, while this audit is `74ca1f1`; CORS/rate-limit/FORCE-RLS catalog/provider-smoke proof remains incomplete | Deploy exact current release candidate and recapture spoof rejection, Auth0 bearer, full RLS/FORCE-RLS, workers/DLQ, secrets/readiness, CORS, rate-limit, and workflow smoke evidence |
| F-2026-07-08-04 MCP money/goods `auto_execute` weakens the strict human-approval floor | Product/security decision required | `apps/backend/app/agents/tools/action_governance.py:321-324` and `352-385` explicitly allow tenant-declared custom tools to auto-fire money/goods/service commitments | Either document this as an accepted tenant-autonomy model with downstream provider controls, or change policy so money/goods/service commitments require Operious approval regardless of MCP `auto_execute` |
| F-2026-07-08-05 DB verification depends on migration discipline | Partially remediated locally | Local `operious_test` was at `0097`; DB-backed tests failed on missing `connector_configs.callback_hmac_secret` until `alembic upgrade head` advanced through `0104_connector_credentials_rls` | Make migration-head verification a required pre-test/release step and rerun full DB suites in CI/normal shell at head |
...
| 25 | Auth error detail can aid recon | Code-closed for the known malformed-auth and malformed-authority cases. Authority-state errors such as disabled header authority, verification unavailable, malformed `Authorization`, and malformed legacy authority headers are coarsened in production. Residual: keep route-specific auth errors from reintroducing detailed production parse or capability oracles. |
...
The work after July 2 is materially stronger than the July 2 state. The committed hardening fixes close several ways a deployment could drift open by default, and the MVP/intelligence-layer plus Phase-2.2 MCP/manager/inbox work is now committed rather than just local scaffolding. The distinction has shifted: current committed code has more real controls, but S-10 live deployment proof is still only partial and is tied to deployed `c65fda5` / Fly v235 rather than audited HEAD `74ca1f1`; current-head type quality is also no longer clean.
...
- Verification quality: prior focused hardening bundle passed `78 passed` and evidence JSON validated. Current-head ruff is clean, but current-head pyright fails with `22 errors, 46 warnings`. `test_system_smoke.py` is non-clean locally because DB/resource-lifespan paths time out, and the 2026-07-05 live smoke reaches provider execution but fails all retries on Anthropic HTTP 400.
...
- Verification quality: focused backend MVP/domain/action tests previously passed `478 passed, 16 skipped`; production LLM/readiness tests passed `44 passed`; supervisor hardening passed `45 passed`; frontend config governance passed `127 passed`; current ruff is clean; current pyright fails; local DB was behind at `0097_mvp7_customer_identity_id` before Codex upgraded the test DB to `0104_connector_credentials_rls` for verification.
...
- Forensic/current residuals: nullable tenant columns plus `row_tenant_id IS NULL OR ...` RLS behavior remain defense-in-depth weaknesses even after the new static invariant; CORS code posture is better but the expanded live capture still reports missing `access-control-allow-origin` on tested preflights; duplicate/dead tracked Python trees and local ignored AWS/Terraform artifacts remain release hygiene blockers; `test_system_smoke.py` remains non-clean under local timeout.
...
- Production proof is partial: spoof rejection, Auth0 `/auth/me`, one-table RLS, readiness, and worker/DLQ evidence are archived for Fly v235 / `c65fda5`, but current-head release proof, full FORCE-RLS catalog evidence, live CORS/rate-limit proof, and provider-completing smoke proof are not complete.
...
- MVP-1 through MVP-9 domain-agnostic and governed-agent work is committed and focused-test proven, but not yet live-provider/prod-composition proven.
...
- Keep generated `__pycache__` bytecode out of commits and treat `apps/backend/scripts/test_http_probe.sh` as an operator helper unless its exact command output is archived as release evidence.
...
| --- | ---: | ---: | --- | --- |
| Governance layer | 72 | 96 | Partial | Durable config ledger, apply attribution, revoke flow, action grants, domain capabilities, object-RBAC sweep, per-tenant action policy, connector/onboarding change requests, connector-specific approval, generalized operation metadata, seeded-replay binding HMAC, approval race guard, custom-tool commitment-kind gates, policy binding, grounding governance, approval auto-delivery, and escalation handoff evidence are strong. The score is capped because MCP `auto_execute` intentionally permits money/goods/service custom tools without an Operious human gate, and live proof/workflow maturity is still missing. |
| Agent layer | 50 | 91 | Partial | Replay is durably actor/payload-bound, connector invocation ledger exists, automatic and manager-approved unconfigured commerce actions now inherit fail-closed stub policy, quota/backpressure are stronger, warranty/replacement and `inventory.check` paths are committed, MCP/generic/custom connector operations are focused-tested, structured extraction is schema-gated, governed LLM/fraud/SOP/QA/supervisor/repair scaffolds exist, and Phase 0 closes production LLM stub fallback. The MCP registration route bug, pyright failures, production provider proof, and full intelligence workflow keep this below the prior score. |
...
| Enterprise readiness | 43 | 84 | Not ready | Security/governance primitives, domain agnosticism, production-fail-closed defaults, compliance docs, and partial S-10 evidence improved readiness. Current-head readiness is capped by S-10 still being open, the failed pyright gate, the MCP registration bug, non-clean smoke/DB full-suite verification, formal compliance operations, DR/SLO evidence, full live CORS/RLS/rate-limit proof, duplicate-tree/repo hygiene, and production provider integrations. |
...
- `docs/pilot-readiness-evidence/2026-07-05-s10-full-capture.json` adds a broader production capture: spoof rejection PASS, Auth0 `/api/v1/auth/me` verified claims PASS, queue/worker health PASS, smoke pipeline operational through diagnostic worker/retries, and production boot gate PASS.
- The same expanded capture is not S-10 closure evidence: CORS preflight is marked `PARTIAL`, rate limiting is only `OBSERVED` after 30 unauthenticated `/health` requests, the smoke still fails final diagnostic completion on Anthropic HTTP 400, and the manifest's exact deployed commit remains `c65fda5` rather than audited HEAD `74ca1f1`.
...
Status: open. The boot gate is committed and partial live production evidence exists, but S-10 is not closed for current HEAD `74ca1f1`.
...
- Prior pyright passed with `0 errors, 0 warnings, 0 informations`, but current-head pyright now fails with `22 errors, 46 warnings`; current ruff passes; S-10 manifest and smoke trace JSON validate as historical partial evidence only.

Untested or insufficiently proven:

- Current-head live production Auth0 `/auth/me` token claim mapping and role assignment. A 2026-07-05 proof exists inside the expanded Fly v235 / `c65fda5` bundle, not for audited HEAD `74ca1f1`.
- Current-head production trusted ingress/direct spoof rejection. A 2026-07-05 proof exists for Fly v235 / `c65fda5`, not for audited HEAD `74ca1f1`.
- Full production FORCE-RLS catalog proof. A 2026-07-05 one-table RLS proof exists, but it is not full catalog evidence.
- Current-head production worker/DLQ/health checks. A 2026-07-05 worker/DLQ proof exists for Fly v235 / `c65fda5`, not for audited HEAD `74ca1f1`.
- Live production SSRF/egress proof.
- Current-head auth/coarsening live proof.
- Current-head CORS allowed/disallowed preflight proof. The expanded 2026-07-05 capture marks CORS preflight `PARTIAL` because `access-control-allow-origin` was absent for tested allowed and disallowed origins.
- Current-head real-client-IP rate-limit bucket proof. The expanded 2026-07-05 capture sent 30 unauthenticated `/health` requests and observed `200`, which does not prove protected-route or real-client-IP bucket behavior.
...
| --- | --- | ---: | ---: | --- |
| Internal demo | Ready with caveats | n/a | 92 | Good if demo/stub boundaries are disclosed, the MCP registration bug is avoided/fixed, and S-10 gaps are not represented as complete. |
| Scoped non-regulated pilot | Conditional | n/a | 84 | Reasonable only after the current-head MCP registration bug and pyright failures are fixed, Phase E checks pass, risky integrations stay constrained, provider drills are bounded, and pilot scope is explicit. |
| $100k customer | Conditional pilot | 58 | 81 | More plausible than baseline after Phase B/Stage 3 and current MVP work, but not ready to sell as production until live proof, current-head quality gates, constrained integrations, release hygiene, and provider drills are clean. |
...
Scoped-pilot safety verdict: not ready at current `74ca1f1` without fixes. It becomes conditionally safe for a narrow, non-regulated pilot only if the MCP registration bug and pyright failures are fixed, Phase E/S-10 checks are recaptured against current HEAD, AWS/Terraform local artifacts are cleaned or quarantined, generated bytecode remains absent, irreversible provider actions stay constrained behind proven connector/idempotency paths, tenant-config DB proof is promoted to CI/normal-shell evidence, live provider drills pass, CORS/rate-limit evidence is clean, and operators do not present S-10 or compliance evidence as complete. It is not yet safe for broad enterprise production.
...
3. MCP or generic connector onboarding goes live before the MCP registration route bug, pyright failures, and money/goods `auto_execute` policy decision are resolved.
4. Real refund/repair/warranty/replacement/warehouse connector connected before provider idempotency, callback signatures, and action side-effect tests.
5. Local AWS CLI archive/vendor tree or placeholder Terraform scaffold accidentally committed/deployed.
6. Voice enabled before production token issuance and real STT/TTS provider proof are complete.
7. Rate-limit per-IP assumptions wrong in production, causing proxy-wide throttling rather than true abusive-client throttling.
8. RAG poisoning through approved-but-poorly-reviewed knowledge despite baseline quarantine controls.
9. XSS in command center leading to token/localStorage authority theft.
10. Compromised integration webhook route flooding queues/nonces; rate limits now reduce but do not eliminate this risk.
...
Score movement after `74ca1f1`: Security moves down from the prior `98` to `96` because the new MCP surface is real but includes an unresolved registration-route bug, an explicit money/goods `auto_execute` policy decision, and no current-head S-10 closure. Enterprise readiness moves down from `86` to `84` because pyright is no longer clean, full DB/smoke verification was not completed cleanly, and S-10 still lacks current-head CORS, rate-limit, FORCE-RLS catalog, provider-smoke, and release-tied proof.
