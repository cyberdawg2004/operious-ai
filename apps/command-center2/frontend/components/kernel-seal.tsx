...
export function KernelSeal({ size = 32, className, phase = 3 }: KernelSealProps) {
  const cx = size / 2;
  const cy = size / 2;
  ...
  return (
    ...
    >
      ...
      {/* Background plate */}
      <rect x={0} y={0} width={size} height={size} rx={size * 0.18} fill={C.surface} />
...
// Full Lockup component for sidebar
...
export function Lockup({ iconSize = 72, phase = 3, theme = "dark", className }: LockupProps) {
  ...
  return (
    <div className={cn("flex items-center", className)} style={{ gap: iconSize * 0.3 }}>
      <KernelSeal size={iconSize} phase={phase} />
      <div className="flex flex-col" style={{ gap: iconSize * 0.055 }}>
        {/* Name row */}
