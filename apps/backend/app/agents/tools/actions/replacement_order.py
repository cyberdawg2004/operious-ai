"""Stub replacement-order action tool."""
...
from app.agents.tools.action_payloads import ReplacementOrderPayload
    ...
    async def invoke(
        ...
        if tenant_id is None or session_id is None:
            return invalid_context_result(
                "replacement.order requires tenant_id and session_id"
            )
        ...
        return success_result(
            ...
            result_summary=(
                f"Replacement order prepared for order {payload.order_id} (stub)"
            ),
            metadata={
