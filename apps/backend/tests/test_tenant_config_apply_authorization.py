...
``require_config_apply_authorization_for`` now keeps that old direct path
...
def _admin(*, approver: bool = False) -> AuthorityContext:
    caps = {TENANT_POLICY_WRITE_CAPABILITY}
    if approver:
...
def _policy_apply_dependency():
    return require_config_apply_authorization_for(TENANT_POLICY_WRITE_CAPABILITY)
