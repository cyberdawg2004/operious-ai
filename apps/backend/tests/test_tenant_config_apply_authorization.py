...
``require_config_apply_authorization`` closes that: in production a
caller may PROPOSE with ``tenant_admin`` but may only APPLY (self-
approve) if they additionally hold the distinct ``tenant.config.approve``
capability — which an organisation grants separately. Lacking it fails
closed with 403 ``independent_approval_required``.
"""
...
def test_production_requires_distinct_approve_capability(
    ...
    assert (
        exc.value.detail["code"] == ERROR_CODE_INDEPENDENT_APPROVAL_REQUIRED
    )


def test_production_allows_when_approve_capability_held(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ENVIRONMENT", "production")
    get_settings.cache_clear()
    authority = _admin(approver=True)
    assert require_config_apply_authorization(authority) is authority


def test_explicit_self_approval_override_in_production(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ENVIRONMENT", "production")
